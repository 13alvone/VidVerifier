...
def fetch_matching_emails(mail, allowed_senders: List[str]) -> List[Tuple[str, List[str], bool]]:
        ...
        logging.info(f"[i] Found {len(message_nums)} unread messages")
        ...
        for num in message_nums:
                ...
                if from_field not in allowed_senders:
                        logging.debug(f"[i] Skipping email from {from_field}")
                        continue
                ...
                logging.debug(f"[i] Processing email '{subject}' from {from_field}")
        ...
        if msg.is_multipart():
            ...
            body = msg.get_payload(decode=True).decode(errors="ignore")
                ...
                if urls:
                        ...
                        logging.debug("[i] No supported URLs found in email")
